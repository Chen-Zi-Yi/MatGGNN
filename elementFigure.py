...
#              'Ba': 4, 'La': 4, 'Hf': 4, 'Og': 1}
json_file = 'oqmd_50w_formation_energy.json'
with open(json_file, 'r') as fp:
    data = json.load(fp)['atom_count']
    print(data)
plot_data = data
maxnum = max(plot_data.values())
print(maxnum)
# 元素周期表中cell的设置
# cell的大小
cell_length = 1
...
norm = mpl.colors.LogNorm(1, maxnum, clip=True)
# 设置超出颜色条下界限的颜色（None为不设置，即白色）
my_cmap.set_under('None')
# 关联颜色条和映射
cmmap = cm.ScalarMappable(norm, my_cmap)
# 绘制颜色条
plt.colorbar(cmmap, ax=plt.gca(), drawedges=False)
...
for e in elements:
    ...
    plt.text(x + 0.04, y + 0.8,
             ele_number,
             va='center', ha='left',
             fontdict={'size': 6, 'color': 'black', 'family': 'Microsoft YaHei'})
    # 在cell中添加元素符号
    plt.text(x + 0.5, y + 0.5,
             ele_symbol,
             va='center', ha='center',
             fontdict={'size': 9, 'color': 'black', 'family': 'Microsoft YaHei', 'weight': 'bold'})
    # 在cell中添加热力值
...
# 将图保存为*.svg矢量格式
plt.savefig('./oqmd_50w_formation_energy.svg')
# 显示绘图窗口
plt.show()